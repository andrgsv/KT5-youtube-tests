...
Проект оформлен в структуре, похожей на пример `sa-teach/selenium-tests`: документация лежит в `docs`, Page Object-классы — в `pages`, автотесты — в `tests`.
...
YouTube — внешний сайт, поэтому интерфейс, локаторы, cookie-окна и региональные настройки могут изменяться. Если тесты падают, сначала проверьте, не изменился ли интерфейс сайта, и при необходимости обновите локаторы в папке `pages`.
